	...
	if($count > 0)
	{
       /*echo "<table align = 'center' width = '40%' border ='1'>";*/
        ...
        while($row = mysqli_fetch_array($query))
        {
				...
				echo"<td width='40%'><b>{$row['book_name']}</b> <br /> {$row['book_description']} <br /><br /> Category: {$row['book_category']} <br /> Publishing Date: {$row['book_date']} <br /> Price: RM{$row['book_retail_price']} <br /><a href ='wishlist.php?wishlist=".$row['book_isbn13']."'><button class='wish'>Add to Wishlist</button></a> <br /><br />";
